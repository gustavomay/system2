...
$consulta = $conn->query("SELECT * FROM infos WHERE idinfos = $id");

while ($info = $consulta->fetch(PDO::FETCH_ASSOC)) {  
    $id = $info['idinfos'];
    $forne = $info['fornecedor'];
}
...
echo '</br>';
echo '<h4>Deseja realmente excluir este Fornecedor ? <b>'.$forne.'</b></h4>';
echo '<labe>ID: </label><input type="text" name="idinfos" id="idinfos" value='.$id.' size="1" readonly="readonly">';
echo '</br>';
echo '<button type="submit" class="btn btn-success">Sim</button>';
echo '<a class="btn btn btn-danger" href="dados_ativos.php">Não</a>';
echo '</form>';
echo '</div>';
